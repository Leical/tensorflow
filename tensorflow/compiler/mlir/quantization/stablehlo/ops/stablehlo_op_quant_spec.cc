...
// `Granularity.dimension_specs` set, which represents the quantization
// dimension for the input, then the corresponding operand index -> quantization
// dimension mapping is set for `spec`.
void PopulateCoeffOpQuantDimIfPerChannelQuantized(
  ...
  if (method.ok() && method->has_static_range_ptq()) {
    ...
    for (const auto& [operand_idx, quantized_type] :
      ...
      if (quantized_type.granularity().has_dimension_specs()) {
        spec.coeff_op_quant_dim[operand_idx] =
            quantized_type.granularity().dimension_specs().dimension();
      }
    }
  }
}
